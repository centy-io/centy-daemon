...
fn classify(lines: &[String]) -> &'static str {
    for l in lines {
        ...
        if t.starts_with("impl<") && t.contains("Service<") {
            return "service";
        }
...
fn split_pub_mod(
    ...
    let raw_body: Vec<&str> = refs[decl_start + 1..refs.len().saturating_sub(1)].to_vec();
    ...
    let prefix: String = refs[..decl_start]
        .iter()
        .map(|l| format!("{l}\n"))
        .collect();
...
pub fn write_maybe_split(
    ...
    match classify(lines) {
        ...
        "service" => {
            ...
            let (condensed, handler_incs) = transform_service_impl(out_dir, stem, lines)?;
            // Write handler includes into a separate file so sub.rs stays ≤99 lines.
            let handlers_file = format!("{stem}.handlers.rs");
            std::fs::write(out_dir.join(&handlers_file), handler_incs.join("\n") + "\n")?;
            let inc_line = format!(r#"include!(concat!(env!("OUT_DIR"), "/{handlers_file}"));"#);
