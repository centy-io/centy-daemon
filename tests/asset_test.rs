...
use centy_daemon::item::entities::issue::{
    add_asset, create_issue, delete_asset, get_asset, list_assets, list_shared_assets, AssetError,
    AssetScope, CreateIssueOptions,
};
...
fn create_test_png() -> Vec<u8> {
    ...
    vec![
        ...
        0xC4, 0x89, 0x00, 0x00, 0x00, 0x0A, 0x49, 0x44, 0x41, // IDAT chunk
...
fn create_test_jpeg() -> Vec<u8> {
    ...
    vec![
        ...
        0x49, 0x46, 0x00, 0x01, 0x01, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0xFF, 0xDB, 0x00,
        0x43, // Quantization table
        0x00, 0x08, 0x06, 0x06, 0x07, 0x06, 0x05, 0x08, 0x07, 0x07, 0x07, 0x09, 0x09, 0x08, 0x0A,
        0x0C, 0x14, 0x0D, 0x0C, 0x0B, 0x0B, 0x0C, 0x19, 0x12, 0x13, 0x0F, 0x14, 0x1D, 0x1A, 0x1F,
        0x1E, 0x1D, 0x1A, 0x1C, 0x1C, 0x20, 0x24, 0x2E, 0x27, 0x20, 0x22, 0x2C, 0x23, 0x1C, 0x1C,
        0x28, 0x37, 0x29, 0x2C, 0x30, 0x31, 0x34, 0x34, 0x34, 0x1F, 0x27, 0x39, 0x3D, 0x38, 0x32,
        0x3C, 0x2E, 0x33, 0x34, 0x32, 0xFF, 0xC0, 0x00, 0x0B, 0x08, 0x00,
        0x01, // Start of frame
        ...
        0x05, 0x04, 0x04, 0x00, 0x00, 0x01, 0x7D, 0xFF, 0xDA, 0x00, 0x08, // Start of scan
        0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7F, 0xFF, 0xD9, // End of image
    ]
}
...
async fn test_add_asset_already_exists() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        AssetError::AssetAlreadyExists(_)
    ));
}
...
async fn test_add_asset_unsupported_file_type() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        AssetError::UnsupportedFileType(_)
    ));
}
...
async fn test_list_shared_assets() {
    ...
    // List shared assets
    let assets = list_shared_assets(project_path).await.expect("Should list");
...
async fn test_delete_asset_removes_file() {
    ...
    assert!(
        asset_path.exists(),
        "Asset file should exist after creation"
    );
    ...
    assert!(
        !asset_path.exists(),
        "Asset file should not exist after deletion"
    );
}
