...
pub(super) fn build_config(req: CreateItemTypeRequest) -> ItemTypeConfig {
    let identifier = if req.identifier == "slug" { IdStrategy::Slug } else { IdStrategy::Uuid };
    ...
    ItemTypeConfig {
        ...
        default_status: if req.default_status.is_empty() { None } else { Some(req.default_status) },
        priority_levels: if req.priority_levels == 0 { None } else { Some(req.priority_levels) },
        custom_fields: req.custom_fields.into_iter().map(|f| CustomFieldDef {
