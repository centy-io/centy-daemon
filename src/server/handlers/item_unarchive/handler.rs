...
pub async fn unarchive_item(
    ...
    let (archived_type, archived_config) =
        match resolve_item_type_config(project_path, ARCHIVED_FOLDER).await {
            Ok(pair) => pair,
            Err(e) => return Ok(Response::new(UnarchiveItemResponse {
                success: false, error: to_error_json(&req.project_path, &e), ..Default::default()
            })),
        };
    ...
    let target_folder =
        match resolve_target_folder(&req.project_path, &archived_item, &req.target_item_type) {
            Ok(folder) => folder,
            Err(error_json) => return Ok(Response::new(UnarchiveItemResponse {
                success: false, error: error_json, ..Default::default()
            })),
        };
    ...
    move_and_respond(
        ...
        &archived_type, &archived_config,
        &target_type, &target_config,
        &req.item_id, target_folder,
    ).await
}
