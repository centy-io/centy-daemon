...
pub async fn get_asset(req: GetAssetRequest) -> Result<Response<GetAssetResponse>, Status> {
    ...
    let issue_id = if req.issue_id.is_empty() { None } else { Some(req.issue_id.as_str()) };
    match crate::item::entities::issue::get_asset(project_path, issue_id, &req.filename, req.is_shared).await {
        Ok((data, asset_info)) => Ok(Response::new(GetAssetResponse {
            success: true, error: String::new(), data, asset: Some(asset_info_to_proto(&asset_info)),
        })),
        Err(e) => Ok(Response::new(GetAssetResponse {
            success: false, error: to_error_json(&req.project_path, &e), data: vec![], asset: None,
        })),
    }
}

pub async fn list_shared_assets(req: ListSharedAssetsRequest) -> Result<Response<ListAssetsResponse>, Status> {
    track_project_async(req.project_path.clone());
    let project_path = Path::new(&req.project_path);
    if let Err(e) = assert_initialized(project_path).await {
        return Ok(Response::new(ListAssetsResponse {
