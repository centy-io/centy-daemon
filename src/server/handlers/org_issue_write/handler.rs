...
use crate::registry::{
    create_org_issue, delete_org_issue, get_org_config, update_org_issue,
    UpdateOrgIssueOptions,
};
use crate::server::proto::{
    CreateOrgIssueRequest, CreateOrgIssueResponse, DeleteOrgIssueRequest,
    DeleteOrgIssueResponse, UpdateOrgIssueRequest, UpdateOrgIssueResponse,
};
...
pub async fn create_org_issue_handler(
    ...
    let config = get_org_config(&req.organization_slug).await.unwrap_or_default();
    let priority = if req.priority == 0 {
        default_priority(config.priority_levels)
    } else {
        req.priority as u32
    };
    let status = if req.status.is_empty() { "open".to_string() } else { req.status.clone() };
    match create_org_issue(
        &req.organization_slug, &req.title, &req.description,
        priority, &status, req.custom_fields, req.referenced_projects,
    ).await {
        Ok(issue) => Ok(Response::new(CreateOrgIssueResponse {
            success: true, error: String::new(),
            id: issue.id.clone(), display_number: issue.display_number,
            created_files: vec![format!(
                "~/.centy/orgs/{}/issues/{}.md", req.organization_slug, issue.id
            )],
        })),
        Err(e) => Ok(Response::new(CreateOrgIssueResponse {
            success: false, error: to_error_json(&req.organization_slug, &e),
            id: String::new(), display_number: 0, created_files: vec![],
        })),
    }
}
pub async fn update_org_issue_handler(
    ...
    let opts = UpdateOrgIssueOptions {
        ...
        status: if req.status.is_empty() { None } else { Some(req.status.clone()) },
        priority: if req.priority == 0 { None } else { Some(req.priority as u32) },
        custom_fields: if req.custom_fields.is_empty() { None } else { Some(req.custom_fields) },
    ...
    match update_org_issue(&req.organization_slug, &req.issue_id, opts).await {
        ...
        Err(e) => Ok(Response::new(UpdateOrgIssueResponse {
            success: false, error: to_error_json(&req.issue_id, &e), issue: None,
        })),
    }
}
pub async fn delete_org_issue_handler(
    ...
    match delete_org_issue(&req.organization_slug, &req.issue_id).await {
        Ok(()) => Ok(Response::new(DeleteOrgIssueResponse { success: true, error: String::new() })),
        Err(e) => Ok(Response::new(DeleteOrgIssueResponse {
            success: false, error: to_error_json(&req.issue_id, &e),
        })),
    }
}
