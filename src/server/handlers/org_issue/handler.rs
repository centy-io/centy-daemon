...
use crate::server::proto::{
    ...
    GetOrgIssueRequest, ListOrgIssuesRequest, ListOrgIssuesResponse,
    OrgConfig as ProtoOrgConfig, OrgIssue as ProtoOrgIssue,
};
...
pub async fn get_org_issue_by_display_number_handler(
    ...
    match get_org_issue_by_display_number(&req.organization_slug, req.display_number).await {
        Ok(issue) => Ok(Response::new(org_issue_to_proto(&issue, config.priority_levels))),
        Err(e) => Err(Status::not_found(to_error_json(&req.display_number.to_string(), &e))),
    }
}
pub async fn list_org_issues_handler(
    ...
    match list_org_issues(&req.organization_slug, opts).await {
        Ok(issues) => {
            ...
            let proto_issues = issues.iter()
                .map(|i| org_issue_to_proto(i, config.priority_levels)).collect();
            Ok(Response::new(ListOrgIssuesResponse { issues: proto_issues, total_count }))
        }
...
pub async fn get_org_config_handler(
    ...
    match get_org_config(&req.organization_slug).await {
        Ok(config) => Ok(Response::new(ProtoOrgConfig {
            ...
            custom_fields: config.custom_fields.into_iter().map(|f| CustomFieldDefinition {
                name: f.name, default_value: f.default_value.unwrap_or_default(),
                field_type: "string".to_string(), required: false, enum_values: vec![],
            }).collect(),
        })),
        Err(e) => Err(Status::internal(to_error_json(&req.organization_slug, &e))),
    }
