...
pub(super) async fn build_ok_response(
    result: mdstore::MoveResult, source_path: &Path,
    target_path: &Path, target_type: &str,
) -> MoveItemResponse {
...
pub(super) async fn finish_move(
    ...
    source_path: &Path, target_path: &Path, target_type: &str,
    hook_type: &str, hook_project_path: &str, hook_item_id: &str,
    hook_request_data: serde_json::Value, error_project_path: &str,
) -> Result<Response<MoveItemResponse>, Status> {
    let success = move_result.is_ok();
    maybe_run_post_hooks(
        source_path, hook_type, HookOperation::Move,
        hook_project_path, Some(hook_item_id), Some(hook_request_data), success,
    ).await;
    match move_result {
        ...
        Err(e) => Ok(Response::new(MoveItemResponse {
            success: false, error: to_error_json(error_project_path, &e), ..Default::default()
        })),
    }
}
/// Assert both source and target projects are initialized.
pub(super) async fn assert_both_initialized(
    req: &MoveItemRequest, source_path: &Path, target_path: &Path,
...
pub(super) async fn resolve_configs(
    ...
    let source = resolve_item_type_config(source_path, &req.item_type).await.map_err(|e| {
        Response::new(MoveItemResponse {
            success: false, error: to_error_json(&req.source_project_path, &e), ..Default::default()
        })
    })?;
    let target = resolve_item_type_config(target_path, &req.item_type).await.map_err(|e| {
        Response::new(MoveItemResponse {
            success: false, error: to_error_json(&req.target_project_path, &e), ..Default::default()
        })
    })?;
