...
pub async fn init(req: InitRequest) -> Result<Response<InitResponse>, Status> {
    ...
    match execute_reconciliation(project_path, decisions, req.force).await {
        Ok(result) => {
            let existing_org = get_project_info(&req.project_path).await.ok()
                .flatten().and_then(|info| info.organization_slug);
            ...
            if let Some(ref proto_config) = req.init_config {
                if let Err(e) = apply_init_config(project_path, proto_config).await {
                    return Ok(Response::new(InitResponse {
                        success: false, error: format!("Failed to write init config: {e}"),
                        created: vec![], restored: vec![], reset: vec![], skipped: vec![],
                        manifest: None, org_inference: None,
                    }));
                }
            }
            if !req.title.is_empty() {
                if let Err(e) = set_project_title(project_path, Some(req.title.clone())).await {
                    return Ok(Response::new(InitResponse {
                        success: false, error: format!("Failed to write project title: {e}"),
                        created: vec![], restored: vec![], reset: vec![], skipped: vec![],
                        manifest: None, org_inference: None,
                    }));
                }
            ...
            Ok(Response::new(InitResponse {
                ...
                reset: result.reset, skipped: result.skipped,
...
pub async fn is_initialized(req: IsInitializedRequest) -> Result<Response<IsInitializedResponse>, Status> {
    track_project_async(req.project_path.clone());
    let project_path = Path::new(&req.project_path);
    let centy_path = get_centy_path(project_path);
    let manifest_path = centy_path.join(".centy-manifest.json");
    let initialized = manifest_path.exists();
    let centy_path_str = if initialized { centy_path.to_string_lossy().to_string() } else { String::new() };
    Ok(Response::new(IsInitializedResponse { initialized, centy_path: centy_path_str }))
}
#[cfg(test)]
#[path = "../init_tests.rs"]
mod init_tests;
