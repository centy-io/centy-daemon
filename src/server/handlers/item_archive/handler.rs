...
pub async fn archive_item(req: ArchiveItemRequest) -> Result<Response<ArchiveItemResponse>, Status> {
    track_project_async(req.project_path.clone());
    let project_path = Path::new(&req.project_path);
    if let Err(e) = assert_initialized(project_path).await {
        return Ok(Response::new(ArchiveItemResponse {
            success: false, error: to_error_json(&req.project_path, &e), item: None,
        }));
    }
    let (source_type, source_config) =
        match resolve_item_type_config(project_path, &req.item_type).await {
            Ok(pair) => pair,
            Err(e) => return Ok(Response::new(ArchiveItemResponse {
                success: false, error: to_error_json(&req.project_path, &e), item: None,
            })),
        };
    ...
    let move_result = do_move_to_archive(
        project_path, &source_type, &archived_type, &source_config, &archived_config, &req.item_id,
    ).await;
    ...
    match move_result {
        Ok(result) => set_original_item_type_and_respond(
            project_path, &req.project_path, &archived_type, &archived_config,
            &source_type, result.item,
        ).await,
