...
pub async fn create_item(req: CreateItemRequest) -> Result<Response<CreateItemResponse>, Status> {
    ...
    let (item_type, config) = match resolve_item_type_config(project_path, &req.item_type).await {
        Ok(pair) => pair,
        Err(e) => return Ok(Response::new(CreateItemResponse {
            success: false, error: to_error_json(&req.project_path, &e), ..Default::default()
        })),
    };
    ...
    if let Err(e) = maybe_run_pre_hooks(
        ...
        &hook_project_path, None, Some(hook_data.clone()),
    ).await {
        return Ok(Response::new(CreateItemResponse {
            success: false, error: to_error_json(&req.project_path, &e), ..Default::default()
        }));
    ...
    let options = build_options(
        req.title, req.body, nonempty(req.status), nonzero_u32(req.priority), req.custom_fields,
    );
    Ok(Response::new(do_create(
        project_path, &item_type, &config, &hook_type,
        &hook_project_path, hook_data, &req.project_path, options,
    ).await))
}
