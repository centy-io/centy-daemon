...
pub async fn delete_asset(req: DeleteAssetRequest) -> Result<Response<DeleteAssetResponse>, Status> {
    ...
    if let Err(e) = maybe_run_pre_hooks(project_path, "asset", HookOperation::Delete,
    ...
    match delete_asset_fn(project_path, issue_id, &req.filename, req.is_shared).await {
        Ok(result) => {
            maybe_run_post_hooks(project_path, "asset", HookOperation::Delete,
            ...
            Ok(Response::new(DeleteAssetResponse {
                success: true, error: String::new(), filename: result.filename,
                was_shared: result.was_shared, manifest: manifest.map(|m| manifest_to_proto(&m)),
            }))
        }
        Err(e) => {
            maybe_run_post_hooks(project_path, "asset", HookOperation::Delete,
            ...
            Ok(Response::new(DeleteAssetResponse {
                ...
                filename: String::new(), was_shared: false, manifest: None,
            }))
        }
    }
}
