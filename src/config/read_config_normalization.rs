...
#[tokio::test]
async fn test_read_config_strips_legacy_hooks_field() {
    ...
    assert!(
        ...
        "config.json should no longer contain hooks after migration"
    );
}
...
async fn test_read_config_strips_legacy_allowed_states() {
    ...
    fs::create_dir_all(&centy_dir)
        .await
        .expect("Should create .centy dir");
    let config_with_allowed_states = r#"{"allowedStates": ["open", "planning", "in-progress", "closed"], "customFields": [], "defaults": {}, "priorityColors": {}, "priorityLevels": 3, "stateColors": {}}"#;
    let config_path = centy_dir.join("config.json");
...
async fn test_read_config_flat_format_works() {
    ...
    fs::create_dir_all(&centy_dir)
        .await
        .expect("Should create .centy dir");
    let flat_config = r#"{"version": "0.0.1", "priorityLevels": 5, "customFields": [], "defaults": {}, "allowedStates": ["open", "closed"], "defaultState": "open", "stateColors": {}, "priorityColors": {}}"#;
    let config_path = centy_dir.join("config.json");
