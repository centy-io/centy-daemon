...
use tokio::fs;
    ...
    async fn test_read_config_normalizes_missing_hooks() {
        ...
        fs::write(&config_path, config_without_hooks).await.expect("Should write config");
        let config = read_config(temp_dir.path()).await.expect("Should read").expect("Config should exist");
        assert!(config.hooks.is_empty());
        let raw = fs::read_to_string(&config_path).await.expect("Should read file");
        let value: serde_json::Value = serde_json::from_str(&raw).expect("Should parse");
        assert!(value.as_object().unwrap().contains_key("hooks"), "config.json should now contain the hooks key");
        assert!(!value.as_object().unwrap().contains_key("llm"), "config.json should not contain llm config");
    }
    ...
    async fn test_read_config_does_not_rewrite_when_hooks_present_and_no_deprecated_fields() {
        ...
        let config_path = centy_dir.join("config.json");
        fs::write(&config_path, config_with_hooks).await.expect("Should write config");
        let config = read_config(temp_dir.path()).await.expect("Should read").expect("Config should exist");
        assert!(config.hooks.is_empty());
        let raw = fs::read_to_string(&config_path).await.expect("Should read file");
    ...
    async fn test_read_config_strips_legacy_allowed_states() {
        ...
        let config_path = centy_dir.join("config.json");
        fs::write(&config_path, config_with_allowed_states).await.expect("Should write config");
        let config = read_config(temp_dir.path()).await.expect("Should read").expect("Config should exist");
        assert_eq!(config.allowed_states.len(), 4);
        let raw = fs::read_to_string(&config_path).await.expect("Should read file");
        let value: serde_json::Value = serde_json::from_str(&raw).expect("Should parse");
        assert!(!value.as_object().unwrap().contains_key("allowedStates"), "config.json should no longer contain allowedStates after rewrite");
    }
    ...
    async fn test_read_config_flat_format_works() {
        ...
        let config_path = centy_dir.join("config.json");
        fs::write(&config_path, flat_config).await.expect("Should write config");
        let config = read_config(temp_dir.path()).await.expect("Should read").expect("Config should exist");
    ...
    fn test_hooks_always_serialized_even_when_empty() {
        ...
        assert!(json.contains("\"hooks\""), "hooks key should be present in serialized JSON even when empty");
    }
