use super::*;
    ...
    async fn test_execute_reconciliation_creates_centy_folder() {
        ...
        execute_reconciliation(temp_dir.path(), decisions, false).await.expect("Should execute reconciliation");
    ...
    async fn test_execute_reconciliation_creates_managed_files() {
        ...
        let temp_dir = tempdir().expect("Should create temp dir");
        let decisions = ReconciliationDecisions::default();
        let result = execute_reconciliation(temp_dir.path(), decisions, false).await.expect("Should execute reconciliation");
    ...
    async fn test_execute_reconciliation_idempotent() {
        ...
        let temp_dir = tempdir().expect("Should create temp dir");
        let decisions = ReconciliationDecisions::default();
        let result1 = execute_reconciliation(temp_dir.path(), decisions.clone(), false).await.expect("Should execute first time");
        let result2 = execute_reconciliation(temp_dir.path(), ReconciliationDecisions::default(), false).await.expect("Should execute second time");
    ...
    async fn test_execute_reconciliation_force_mode() {
        ...
        let temp_dir = tempdir().expect("Should create temp dir");
        let decisions = ReconciliationDecisions::default();
        let result = execute_reconciliation(temp_dir.path(), decisions, true).await.expect("Should execute with force");
        assert!(!result.created.is_empty());
    }
    ...
    async fn test_execute_reconciliation_skips_modified_without_decision() {
        ...
        let result = execute_reconciliation(temp_dir.path(), ReconciliationDecisions::default(), false).await.expect("Should execute");
    ...
    async fn test_execute_reconciliation_creates_config_json() {
        ...
        assert!(result.created.contains(&"config.json".to_string()), "Should report config.json as created");
        ...
        assert!(value.as_object().unwrap().contains_key("hooks"), "config.json should contain hooks key");
    }
