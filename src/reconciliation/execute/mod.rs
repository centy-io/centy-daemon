...
pub async fn execute_reconciliation(
    ...
    if !centy_path.exists() { fs::create_dir_all(&centy_path).await?; }
    let mut manifest = read_manifest(project_path).await?.unwrap_or_else(create_manifest);
    ...
    for file_info in &plan.to_restore {
        ...
        } else { result.skipped.push(file_info.path.clone()); }
    }
    for file_info in &plan.to_reset {
        let template = managed_templates.get(&file_info.path);
        let has_merge = template.map(|t| t.merge_strategy.is_some()).unwrap_or(false);
        if has_merge {
    ...
    for path in migrated { result.created.push(path); }
