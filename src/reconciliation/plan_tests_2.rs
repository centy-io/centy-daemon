use super::*;
    ...
    async fn test_build_reconciliation_plan_empty_directory() {
        ...
        let plan = build_reconciliation_plan(temp_dir.path()).await.expect("Should build plan");
    ...
    async fn test_scan_centy_folder_with_files() {
        ...
        let temp_dir = tempdir().expect("Should create temp dir");
        let centy_path = temp_dir.path().join(".centy");
        fs::create_dir_all(&centy_path).await.expect("Should create .centy");
        fs::write(centy_path.join("test.txt"), "content").await.expect("Should write file");
        fs::create_dir_all(centy_path.join("subdir")).await.expect("Should create subdir");
        let files = helpers::scan_centy_folder(&centy_path);
    ...
    async fn test_scan_centy_folder_skips_manifest() {
        ...
        let centy_path = temp_dir.path().join(".centy");
        fs::create_dir_all(&centy_path).await.expect("Should create .centy");
        fs::write(centy_path.join(".centy-manifest.json"), "{}").await.expect("Should write manifest");
        fs::write(centy_path.join("README.md"), "content").await.expect("Should write readme");
        let files = helpers::scan_centy_folder(&centy_path);
    ...
    fn test_reconciliation_plan_clone() {
        ...
        plan.to_create.push(FileInfo { path: "test.md".to_string(), file_type: ManagedFileType::File, hash: "abc".to_string(), content_preview: None });
