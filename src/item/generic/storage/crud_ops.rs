...
pub async fn generic_create(
    project_path: &Path, folder: &str, config: &TypeConfig, options: CreateOptions,
...
pub async fn generic_get(project_path: &Path, folder: &str, id: &str) -> Result<mdstore::Item, ItemError> {
    let type_dir = type_storage_path(project_path, folder);
    Ok(mdstore::get(&type_dir, id).await?)
}
/// Get a single generic item by display number.
pub async fn generic_get_by_display_number(
    project_path: &Path, folder: &str, config: &TypeConfig, display_number: u32,
) -> Result<mdstore::Item, ItemError> {
    if !config.features.display_number {
        return Err(ItemError::FeatureNotEnabled("display_number is not enabled for this item type".to_string()));
    }
    let type_dir = type_storage_path(project_path, folder);
    let items = mdstore::list(&type_dir, Filters::new().include_deleted()).await?;
    for item in items {
        if item.frontmatter.display_number == Some(display_number) { return Ok(item); }
    }
    Err(ItemError::NotFound(format!("display_number {display_number}")))
}
/// List generic items with optional filters.
pub async fn generic_list(project_path: &Path, folder: &str, filters: Filters) -> Result<Vec<mdstore::Item>, ItemError> {
    let type_dir = type_storage_path(project_path, folder);
    Ok(mdstore::list(&type_dir, filters).await?)
}
/// Update an existing generic item.
pub async fn generic_update(
    project_path: &Path, folder: &str, config: &TypeConfig, id: &str, options: UpdateOptions,
) -> Result<mdstore::Item, ItemError> {
    let type_dir = type_storage_path(project_path, folder);
...
pub async fn generic_delete(
    project_path: &Path, folder: &str, config: &TypeConfig, id: &str, force: bool,
) -> Result<(), ItemError> {
    let type_dir = type_storage_path(project_path, folder);
    mdstore::delete(&type_dir, id, force).await?;
    if force && config.features.assets {
        let assets_path = get_centy_path(project_path).join("assets").join(folder).join(id);
        if assets_path.exists() { fs::remove_dir_all(&assets_path).await?; }
    }
    update_project_manifest(project_path).await?;
    Ok(())
}
/// Soft-delete an item by setting the `deleted_at` timestamp.
pub async fn generic_soft_delete(project_path: &Path, folder: &str, id: &str) -> Result<(), ItemError> {
    let type_dir = type_storage_path(project_path, folder);
