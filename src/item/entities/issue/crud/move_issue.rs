...
pub async fn move_issue(options: MoveIssueOptions) -> Result<MoveIssueResult, IssueCrudError> {
    ...
    let mut source_manifest = read_manifest(&options.source_project_path)
        .await?.ok_or(IssueCrudError::NotInitialized)?;
    let mut target_manifest = read_manifest(&options.target_project_path)
        .await?.ok_or(IssueCrudError::TargetNotInitialized)?;
    ...
    let (source_is_new_format, source_issue) = if source_file_path.exists() {
        (true, read_issue_from_frontmatter(&source_file_path, &options.issue_id).await?)
    } else if source_folder_path.exists() {
        (false, read_issue_from_legacy_folder(&source_folder_path, &options.issue_id).await?)
    } else {
    ...
    validate_status_for_project(&options.target_project_path, "issues", &source_issue.metadata.status).await?;
    ...
    let issue_content = generate_frontmatter(&frontmatter, &source_issue.title, &source_issue.description);
    ...
    if source_assets_path.exists() {
        ...
        copy_assets_folder(&source_assets_path, &target_assets_path)
            .await.map_err(|e| IssueCrudError::IoError(std::io::Error::other(e.to_string())))?;
    }
    if source_is_new_format {
        fs::remove_file(&source_file_path).await?;
        if source_assets_path.exists() { fs::remove_dir_all(&source_assets_path).await?; }
    } else {
    ...
    Ok(MoveIssueResult { issue: moved_issue, old_display_number, source_manifest, target_manifest })
}
