...
use super::update_helpers::{AppliedIssueUpdates, build_issue_struct, build_update_body, build_updated_metadata};
...
pub async fn update_issue(
    ...
    let new_description = options.description.unwrap_or_else(|| current.description.clone());
    let new_status = options.status.unwrap_or_else(|| current.metadata.status.clone());
    ...
    for (key, value) in options.custom_fields { new_custom_fields.insert(key, value); }
    let updates = AppliedIssueUpdates {
        title: new_title, description: new_description, status: new_status,
        priority: new_priority, custom_fields: new_custom_fields,
        draft: options.draft.unwrap_or(current.metadata.draft),
    };
    let updated_metadata = build_updated_metadata(&current, &updates);
    let frontmatter = IssueFrontmatter::from_metadata(&updated_metadata, updates.custom_fields.clone());
    ...
    let body = build_update_body(&old_status, &updates.status, &updates.description, &current_content);
    ...
    let issue = build_issue_struct(issue_number, &updates, &current, &updated_metadata.common.updated_at);
