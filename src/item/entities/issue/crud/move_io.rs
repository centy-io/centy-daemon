...
pub(super) async fn load_source_issue(
    ...
    issue_id: &str,
) -> Result<(bool, Issue, PathBuf, PathBuf), IssueCrudError> {
    let file_path = source_issues_path.join(format!("{issue_id}.md"));
    let folder_path = source_issues_path.join(issue_id);
    if file_path.exists() {
        let issue = read_issue_from_frontmatter(&file_path, issue_id).await?;
        Ok((true, issue, file_path, folder_path))
    } else if folder_path.exists() {
        let issue = read_issue_from_legacy_folder(&folder_path, issue_id).await?;
        Ok((false, issue, file_path, folder_path))
    } else {
