...
impl OrgSyncable for Issue {
    ...
    async fn sync_to_project(
        ...
        create_issue_in_project(
            target_project, &self.id, &self.title, &self.description, &self.metadata, org_slug,
        ).await
    }
...
pub async fn create_issue_in_project(
    ...
    if issue_file_path.exists() || issue_folder_path.exists() { return Ok(()); }
    fs::create_dir_all(&issues_path).await?;
    let local_display_number = get_next_display_number(&issues_path)
        .await.map_err(|e| OrgSyncError::SyncFailed(e.to_string()))?;
    let now = now_iso();
    let frontmatter = IssueFrontmatter {
        ...
        deleted_at: None, is_org_issue: true,
