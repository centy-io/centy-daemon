...
pub async fn update_doc(project_path: &Path, slug: &str, options: UpdateDocOptions) -> Result<UpdateDocResult, DocError> {
    ...
    if !doc_path.exists() { return Err(DocError::DocNotFound(slug.to_string())); }
    ...
    let new_slug = match options.new_slug {
        Some(s) if !s.trim().is_empty() && s != slug => {
            ...
            if new_path.exists() { return Err(DocError::SlugAlreadyExists(new_slug)); }
            Some(new_slug)
        }
        _ => None,
    };
    let updated_metadata = DocMetadata {
        created_at: current.metadata.created_at.clone(), updated_at: now_iso(),
        deleted_at: current.metadata.deleted_at.clone(),
        is_org_doc: current.metadata.is_org_doc, org_slug: current.metadata.org_slug.clone(),
    };
    ...
    let final_slug = if let Some(ref new_slug) = new_slug {
        ...
        new_slug.clone()
    } else { fs::write(&doc_path, &doc_content).await?; slug.to_string() };
    update_manifest(&mut manifest);
    write_manifest(project_path, &manifest).await?;
    let doc = Doc { slug: final_slug.clone(), title: new_title.clone(), content: new_content.clone(), metadata: updated_metadata };
    let sync_results = if doc.metadata.is_org_doc {
        if let Some(ref org) = doc.metadata.org_slug {
            sync_org_doc_update_to_projects(org, project_path, &final_slug, &new_title, &new_content, new_slug.as_ref().map(|_| slug)).await
        } else { Vec::new() }
    } else { Vec::new() };
...
pub async fn sync_org_doc_update_to_projects(org_slug: &str, source_project_path: &Path, slug: &str, title: &str, content: &str, old_slug: Option<&str>) -> Vec<OrgDocSyncResult> {
    ...
    for project in org_projects {
        let result = update_or_create_doc_in_project(Path::new(&project.path), slug, title, content, org_slug, old_slug).await;
...
pub async fn update_or_create_doc_in_project(project_path: &Path, slug: &str, title: &str, content: &str, org_slug: &str, old_slug: Option<&str>) -> Result<(), DocError> {
    ...
    let metadata = if doc_path.exists() {
        ...
        DocMetadata { created_at: existing.metadata.created_at, updated_at: now_iso(), deleted_at: None, is_org_doc: true, org_slug: Some(org_slug.to_string()) }
    } else {
        ...
        DocMetadata { created_at: old_created_at.unwrap_or_else(now_iso), updated_at: now_iso(), deleted_at: None, is_org_doc: true, org_slug: Some(org_slug.to_string()) }
    };
    fs::write(&doc_path, crate::utils::format_markdown(&generate_doc_content(title, content, &metadata))).await?;
    update_manifest(&mut manifest);
    write_manifest(project_path, &manifest).await?;
    Ok(())
}
