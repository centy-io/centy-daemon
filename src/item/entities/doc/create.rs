...
/// Create a new doc
pub async fn create_doc(project_path: &Path, options: CreateDocOptions) -> Result<CreateDocResult, DocError> {
    if options.title.trim().is_empty() { return Err(DocError::TitleRequired); }
    ...
    let slug = match options.slug {
        Some(s) if !s.trim().is_empty() => { let slug = slugify(&s); validate_slug(&slug)?; slug }
        _ => slugify(&options.title),
    };
    let doc_path = docs_path.join(format!("{slug}.md"));
    if doc_path.exists() { return Err(DocError::SlugAlreadyExists(slug)); }
    let org_slug = if options.is_org_doc {
        let project_path_str = project_path.to_string_lossy().to_string();
        let project_info = get_project_info(&project_path_str).await
    ...
    let metadata = if let Some(ref org) = org_slug { DocMetadata::new_org_doc(org) } else { DocMetadata::new() };
    let doc_content = if let Some(ref template_name) = options.template {
        let template_engine = TemplateEngine::new();
        let context = DocTemplateContext {
            title: options.title.clone(), content: options.content.clone(), slug: slug.clone(),
            created_at: metadata.created_at.clone(), updated_at: metadata.updated_at.clone(),
        };
        template_engine.render_doc(project_path, template_name, &context).await?
    } else {
    ...
    Ok(CreateDocResult { slug, created_file, manifest, sync_results })
}

/// Sync an org doc to all other projects in the organization
pub async fn sync_org_doc_to_projects(
    org_slug: &str, source_project_path: &Path, slug: &str, title: &str, content: &str,
...
pub async fn create_doc_in_project(project_path: &Path, slug: &str, title: &str, content: &str, org_slug: &str) -> Result<(), DocError> {
    let mut manifest = read_manifest(project_path).await?.ok_or(DocError::NotInitialized)?;
    let centy_path = get_centy_path(project_path);
    let docs_path = centy_path.join("docs");
    if !docs_path.exists() { fs::create_dir_all(&docs_path).await?; }
    let doc_path = docs_path.join(format!("{slug}.md"));
    if doc_path.exists() { return Err(DocError::SlugAlreadyExists(slug.to_string())); }
