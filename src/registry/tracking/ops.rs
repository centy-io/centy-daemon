...
pub async fn track_project(project_path: &str) -> Result<(), RegistryError> {
    ...
    let needs_org_inference: bool;
    {
        ...
        if let Some(entry) = registry.projects.get_mut(&canonical_path) {
            ...
            registry.projects.insert(canonical_path.clone(), TrackedProject { first_accessed: now.clone(), last_accessed: now.clone(), is_favorite: false, is_archived: false, organization_slug: None, user_title: None });
    ...
    if needs_org_inference {
        ...
        tokio::spawn(async move { let _ = try_auto_assign_organization(&path_for_inference, None).await; });
    }
    Ok(())
}
...
pub fn track_project_async(project_path: String) {
    tokio::spawn(async move {
        if let Err(e) = track_project(&project_path).await { warn!("Failed to track project {}: {}", project_path, e); }
    });
}
...
pub async fn untrack_project(project_path: &str) -> Result<(), RegistryError> {
    ...
    let _guard = get_lock().lock().await;
    let mut registry = read_registry().await?;
    if registry.projects.remove(&canonical_path).is_none() && registry.projects.remove(project_path).is_none() {
...
pub async fn set_project_favorite(project_path: &str, is_favorite: bool) -> Result<ProjectInfo, RegistryError> {
    let canonical_path = Path::new(project_path).canonicalize().map_or_else(|_| project_path.to_string(), |p| p.to_string_lossy().to_string());
    let _guard = get_lock().lock().await;
    let mut registry = read_registry().await?;
    let tracked = if let Some(t) = registry.projects.get_mut(canonical_path.as_str()) { t }
        else if let Some(t) = registry.projects.get_mut(project_path) { t }
        else { return Err(RegistryError::ProjectNotFound(project_path.to_string())); };
    tracked.is_favorite = is_favorite;
    let tracked_clone = tracked.clone();
    let org_name = tracked_clone.organization_slug.as_ref().and_then(|slug| registry.organizations.get(slug)).map(|org| org.name.clone());
    registry.updated_at = now_iso();
    write_registry_unlocked(&registry).await?;
    Ok(enrich_project(&canonical_path, &tracked_clone, org_name).await)
}
/// Set the archived status for a project
pub async fn set_project_archived(project_path: &str, is_archived: bool) -> Result<ProjectInfo, RegistryError> {
    let canonical_path = Path::new(project_path).canonicalize().map_or_else(|_| project_path.to_string(), |p| p.to_string_lossy().to_string());
    let _guard = get_lock().lock().await;
    let mut registry = read_registry().await?;
    let tracked = if let Some(t) = registry.projects.get_mut(canonical_path.as_str()) { t }
...
pub async fn set_project_user_title(project_path: &str, title: Option<String>) -> Result<ProjectInfo, RegistryError> {
    let canonical_path = Path::new(project_path).canonicalize().map_or_else(|_| project_path.to_string(), |p| p.to_string_lossy().to_string());
    let _guard = get_lock().lock().await;
    let mut registry = read_registry().await?;
    let tracked = if let Some(t) = registry.projects.get_mut(canonical_path.as_str()) { t }
