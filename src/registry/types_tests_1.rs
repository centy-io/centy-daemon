...
fn test_organization_serialization() {
    let org = Organization {
        name: "Acme Corp".to_string(), description: Some("Our company".to_string()),
...
fn test_organization_without_description() {
    let org = Organization {
        name: "Test".to_string(), description: None,
...
fn test_project_organization_serialization() {
    ...
    let deserialized: ProjectOrganization = serde_json::from_str(&json).expect("Should deserialize");
    assert_eq!(deserialized.slug, "acme"); assert_eq!(deserialized.name, "Acme Corp");
}
#[test]
fn test_tracked_project_serialization() {
    let tp = TrackedProject {
        ...
        organization_slug: Some("acme".to_string()), user_title: Some("My Project".to_string()),
    };
    ...
    assert!(deserialized.is_favorite); assert!(!deserialized.is_archived);
...
fn test_tracked_project_defaults() {
    ...
    assert!(!tp.is_favorite); assert!(!tp.is_archived);
    assert!(tp.organization_slug.is_none()); assert!(tp.user_title.is_none());
}
#[test]
fn test_tracked_project_skip_serializing_none() {
    let tp = TrackedProject {
        first_accessed: "2024-01-01".to_string(), last_accessed: "2024-01-01".to_string(),
        is_favorite: false, is_archived: false, organization_slug: None, user_title: None,
    };
    let json = serde_json::to_string(&tp).expect("Should serialize");
    assert!(!json.contains("organizationSlug")); assert!(!json.contains("userTitle"));
}
