...
pub async fn create_link(project_path: &Path, options: CreateLinkOptions, custom_types: &[CustomLinkTypeDefinition]) -> Result<CreateLinkResult, LinkError> {
    ...
    if !entity_exists(project_path, &options.source_id, &options.source_type) { return Err(LinkError::SourceNotFound(options.source_id.clone(), options.source_type.clone())); }
    if !entity_exists(project_path, &options.target_id, &options.target_type) { return Err(LinkError::TargetNotFound(options.target_id.clone(), options.target_type.clone())); }
